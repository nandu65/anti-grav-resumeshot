...
export const ResumeRollingMarquee = memo(function ResumeRollingMarquee() {
  ...
  return (
    <div className="relative w-full overflow-hidden py-10 border-y border-zinc-200/80 dark:border-white/[0.08] bg-slate-50/80 dark:bg-[#090b0e] select-none transition-colors duration-300">
      ...
      <div className="container mx-auto px-4 mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-xs font-bold uppercase tracking-[0.2em] text-zinc-800 dark:text-zinc-300 flex items-center gap-2">
        ...
        <div className="flex items-center gap-4 text-xs text-zinc-600 dark:text-zinc-400">
          <span className="flex items-center gap-1.5"><Sparkles className="h-3.5 w-3.5 text-emerald-500 dark:text-emerald-400" /> Avg ATS Score: <strong className="text-emerald-600 dark:text-emerald-400">96.4%</strong></span>
          <span className="hidden md:inline-flex items-center gap-1.5"><Award className="h-3.5 w-3.5 text-amber-500" /> 12,400+ Resumes Processed</span>
        </div>
      </div>

      {/* Fade masks on left & right */}
      <div className="pointer-events-none absolute inset-y-0 left-0 w-28 sm:w-48 bg-gradient-to-r from-slate-50 dark:from-[#090b0e] via-slate-50/90 dark:via-[#090b0e]/90 to-transparent z-20" />
      <div className="pointer-events-none absolute inset-y-0 right-0 w-28 sm:w-48 bg-gradient-to-l from-slate-50 dark:from-[#090b0e] via-slate-50/90 dark:via-[#090b0e]/90 to-transparent z-20" />
      ...
      <div className="marquee-row flex gap-4 w-max hover:[animation-play-state:paused] mb-4">
        {duplicatedTrack1.map((item, idx) => (
          <div
            key={`t1-${item.id}-${idx}`}
            className="group relative flex flex-col justify-between w-[300px] sm:w-[340px] rounded-2xl bg-white/95 dark:bg-[#11141b]/95 border border-zinc-200/80 dark:border-white/[0.08] hover:border-emerald-500/50 p-4 transition-all duration-300 shadow-md hover:shadow-xl dark:hover:shadow-emerald-500/10 hover:-translate-y-1"
          >
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  <span className="font-bold text-sm text-zinc-900 dark:text-zinc-100 truncate">{item.name}</span>
                  <span className="text-[10px] text-zinc-400 dark:text-zinc-500 font-mono">{item.time}</span>
                </div>
                <div className="text-xs text-zinc-500 dark:text-zinc-400 truncate mt-0.5">{item.role}</div>
                <div className="inline-flex items-center gap-1.5 mt-1.5 text-xs font-semibold text-zinc-700 dark:text-zinc-300">
                  <span className="h-2 w-2 rounded-full inline-block" style={{ backgroundColor: item.companyColor }} />
                  <span>Target: <strong className="text-zinc-900 dark:text-white">{item.company}</strong></span>
                </div>
              </div>

              {/* ATS Match Score Chip */}
              <div className="shrink-0 flex flex-col items-end">
                <div className="flex items-center gap-1 bg-emerald-500/15 border border-emerald-500/30 px-2.5 py-1 rounded-full text-emerald-600 dark:text-emerald-400 font-extrabold text-xs shadow-sm">
                  <CheckCircle2 className="h-3.5 w-3.5 text-emerald-500 dark:text-emerald-400" />
                  <span>{item.score}%</span>
                </div>
                {item.hike && (
                  <span className="text-[10px] text-amber-600 dark:text-amber-300 font-bold mt-1 bg-amber-400/10 px-1.5 py-0.5 rounded border border-amber-400/20">
                    {item.hike}
                  </span>
                )}
              </div>
            ...
            <div className="mt-3 py-1.5 px-2.5 rounded-xl bg-zinc-50 dark:bg-[#161922] border border-zinc-200/60 dark:border-white/[0.04] flex items-center justify-between text-xs">
              ...
              <span className="text-[10px] text-zinc-400 dark:text-zinc-500 font-mono">100% ATS Safe</span>
            </div>

            <div className="mt-2.5 pt-2 border-t border-zinc-100 dark:border-white/[0.06] flex flex-wrap gap-1">
              {item.keywords.map((kw, kIdx) => (
                <span key={kIdx} className="text-[10px] px-2 py-0.5 rounded-md bg-zinc-100 dark:bg-white/[0.04] text-zinc-700 dark:text-zinc-300 border border-zinc-200/60 dark:border-white/[0.06] font-mono">
                  ✓ {kw}
                </span>
              ))}
            </div>
