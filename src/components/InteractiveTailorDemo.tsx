...
export function InteractiveTailorDemo() {
  ...
  return (
    <div className="relative w-full rounded-3xl border border-white/[0.12] bg-[#0d1017]/95 p-6 sm:p-8 shadow-2xl backdrop-blur-2xl overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 pb-6 border-b border-white/[0.08]">
        ...
        {/* Target Role Pills */}
        <div className="flex flex-wrap items-center gap-2 p-1.5 rounded-2xl bg-[#161922] border border-white/[0.06]">
          {DEMO_ROLES.map((r) => {
      ...
      <div className="relative z-10 grid lg:grid-cols-12 gap-6 mt-6 items-stretch">
        ...
        <div className="lg:col-span-4 rounded-2xl border border-white/[0.08] bg-[#11141b] p-5 flex flex-col justify-between shadow-lg">
          <div>
            ...
            {/* Circular Gauge */}
            <div className="flex items-center gap-4 p-4 rounded-xl bg-[#161922] border border-white/[0.06] mb-5">
              <div className="relative h-20 w-20 shrink-0">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  ...
                  <span className="text-[9px] text-zinc-400 font-mono -mt-1">/ 100</span>
                </div>
              </div>
              ...
              <div>
                ...
                <span className="text-[11px] text-zinc-400 block mt-0.5">Target: {selectedRole.role}</span>
            ...
            <div>
              <div className="text-[11px] font-bold uppercase tracking-wider text-zinc-400 mb-2 flex items-center justify-between">
                ...
                <span className="text-emerald-400 font-mono text-[10px]">+{selectedRole.addedKeywords.length} Detected</span>
              </div>
              <div className="flex flex-wrap gap-1.5">
                {selectedRole.addedKeywords.map((kw, i) => (
                  <span
                    key={kw}
                    className="inline-flex items-center gap-1 text-[11px] px-2.5 py-1 rounded-lg bg-emerald-500/10 text-emerald-300 border border-emerald-500/25 font-mono animate-in fade-in zoom-in-95 duration-300"
          ...
          <div className="pt-4 mt-4 border-t border-white/[0.06] flex items-center gap-2 text-xs text-zinc-400">
        ...
        <div className="lg:col-span-8 rounded-2xl border border-white/[0.08] bg-[#11141b] p-5 sm:p-6 flex flex-col justify-between shadow-lg">
          <div>
            {/* View Switcher Tabs */}
            <div className="flex items-center justify-between pb-4 border-b border-white/[0.06] mb-4">
              <div className="flex items-center gap-2">
                <Sparkles className="h-4 w-4 text-emerald-400" />
                <span className="font-bold text-sm text-zinc-100">Experience Bullet Transformation</span>
              </div>
              <div className="flex items-center gap-1 bg-[#161922] p-1 rounded-xl border border-white/[0.06]">
                <button
                  type="button"
                  onClick={() => setActiveTab("after")}
                  className={`px-3 py-1 text-xs font-bold rounded-lg transition-all cursor-pointer ${
                    activeTab === "after" ? "bg-emerald-500 text-slate-950 shadow-sm" : "text-zinc-400 hover:text-white"
                  }`}
                >
                  Tailored (AI)
                </button>
                <button
                  ...
                  className={`px-3 py-1 text-xs font-bold rounded-lg transition-all cursor-pointer ${
                    activeTab === "diff" ? "bg-emerald-500 text-slate-950 shadow-sm" : "text-zinc-400 hover:text-white"
                  }`}
                >
                  Before vs After
                </button>
                <button
                  ...
                  className={`px-3 py-1 text-xs font-bold rounded-lg transition-all cursor-pointer ${
                    activeTab === "before" ? "bg-emerald-500 text-slate-950 shadow-sm" : "text-zinc-400 hover:text-white"
                  }`}
                >
                  Original
                </button>
            ...
            <div className="min-h-[160px] flex flex-col justify-center">
              {activeTab === "after" && (
                <div className="p-4 rounded-xl bg-[#161922] border border-emerald-500/30 text-zinc-100 text-sm sm:text-base leading-relaxed animate-in fade-in duration-300">
                  <div className="flex items-start gap-2.5">
                    <span className="h-5 w-5 rounded-full bg-emerald-500/20 text-emerald-400 flex items-center justify-center shrink-0 mt-0.5 font-bold text-xs">
                      ✓
                    </span>
                    <div>
                      <p className="font-medium text-zinc-100">{selectedRole.afterBullet}</p>
                      <div className="mt-3 flex flex-wrap items-center gap-2 text-[11px] text-emerald-400 font-semibold">
              ...
              {activeTab === "before" && (
                <div className="p-4 rounded-xl bg-[#161922] border border-red-500/20 text-zinc-400 text-sm sm:text-base leading-relaxed animate-in fade-in duration-300">
                  <div className="flex items-start gap-2.5">
                    <span className="h-5 w-5 rounded-full bg-red-500/20 text-red-400 flex items-center justify-center shrink-0 mt-0.5 font-bold text-xs">
                      ✕
                    </span>
                    <div>
                      <p className="line-through opacity-75">{selectedRole.beforeBullet}</p>
                      <div className="mt-3 text-[11px] text-red-400 font-semibold">
              ...
              {activeTab === "diff" && (
                <div className="space-y-3 animate-in fade-in duration-300">
                  <div className="p-3 rounded-xl bg-red-950/20 border border-red-500/20 text-xs text-red-300 flex items-start gap-2">
                    <span className="font-bold shrink-0 text-red-400">- BEFORE:</span>
                    <span className="line-through">{selectedRole.beforeBullet}</span>
                  </div>
                  <div className="p-3 rounded-xl bg-emerald-950/30 border border-emerald-500/30 text-xs text-emerald-200 flex items-start gap-2">
                    <span className="font-bold shrink-0 text-emerald-400">+ AFTER:</span>
          ...
          <div className="pt-4 mt-4 border-t border-white/[0.06] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div className="text-xs text-zinc-400">
              Transform your own resume bullets in <strong className="text-white">under 30 seconds</strong>.
            </div>
